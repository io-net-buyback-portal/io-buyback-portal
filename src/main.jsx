...
<div className="modal" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setModal(false)}>×</button><h3>Confirm Demo Participation</h3><p>Enter BNB Amount</p><input type="number" min="5" step=".01" value={amount} onChange={e=>setAmount(Number(e.target.value))}/><button onClick={()=>{setModal(false)}}>Confirm Demo Participation</button></div></div>}
